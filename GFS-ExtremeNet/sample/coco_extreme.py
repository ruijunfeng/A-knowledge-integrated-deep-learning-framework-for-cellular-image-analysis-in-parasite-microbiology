...
from sample.utils import random_crop_pts, draw_gaussian, gaussian_radius
...
def _clip_detections_pts(image, detections, extreme_pts):
    ...
    # keep the box with positive height and width
...
def kp_detection(db, k_ind, system_configs, data_aug, debug):
    # init params
    ...
    for b_ind in range(batch_size):
        ...
        # mark the next image (% ensure the id do not exceed the size of dataset)
        ...
        # ground truths pre-processing
        for ind, detection in enumerate(detections):
            category = int(detection[-1]) - 1
            extreme_pt = extreme_pts[ind]
            
            # map the ground truth extreme points to the output size
            ...
            # draw a gaussian bump around the ground truth extreme points as the ground truth heatmaps
            if gaussian_bump:
            ...
            # directly use the extreme points as the ground truth heatmaps
            else:
    ...
    if debug:
        ...
        # unormalized and scaled back to 255
...
def sample_data(db, k_ind, system_configs, data_aug=True, debug=False):
    return globals()[system_configs.sampling_function](db, k_ind, system_configs, data_aug, debug)
