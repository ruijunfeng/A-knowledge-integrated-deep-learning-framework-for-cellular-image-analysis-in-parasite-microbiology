...
class Debugger(object):
  def __init__(self, ipynb = False, num_classes=1):
    self.ipynb = ipynb
    if not self.ipynb:
      self.plt = plt
  ...
  def gen_colormap(self, img, s=4):
    ...
    for i in range(num_classes):
      # resize the input image to the size of colormap
      resized = cv2.resize(img[i], (w*s, h*s)).reshape(h*s, w*s, 1)
      # select a color
      cl =  self.colors[i]
      # add it back to the colormap
      color_map = np.maximum(color_map, (2 * resized * cl).astype(np.uint8))
  ...
  def save_all_imgs(self, path='./debug/', prefix='', genID=False):
    if genID:
      try:
